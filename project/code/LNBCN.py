...
for name,G in graphs.items():
    ...
    clusters = nx.clustering(G_train)
    rho = nx.density(G_train)
    for u,v in nx.non_edges(G_train):
        score = 0
        for z in nx.common_neighbors(G_train,u,v):
            c = clusters[z]
            score += math.log(max(eps,c)/max(eps,1-c))+math.log(max(eps,1-rho)/max(eps,rho))
